...
function valueBattleField() {
    ...
    for (const $boardItem of $fieldItem) {
...
for (let i = 0; i < $balls.length; i++) {
    ...
    function addPoints(Points) {
        const verifyActivated = verifyActivatedBall(Points)
        if (winner === "X") {
            pointsP1 = pointsP1 + 1
            if (pointsP1 >= verifyActivated) {
